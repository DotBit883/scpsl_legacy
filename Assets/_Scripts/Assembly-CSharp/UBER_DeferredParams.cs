...
public class UBER_DeferredParams : MonoBehaviour
{
	...
	private bool NotifyDecals()
	{
		...
		{
			bool flag = false;
			if (FindAnyObjectByType(type) != null && FindAnyObjectByType(type) is MonoBehaviour && (UnityEngine.Object.FindObjectOfType(type) as MonoBehaviour).enabled)
			{
				(FindAnyObjectByType(type) as MonoBehaviour)?.Invoke("OnDisable", 0f);
				(FindAnyObjectByType(type) as MonoBehaviour)?.Invoke("OnEnable", 0f);
				return true;
			}
		}
		return false;
